# visualization of the RF model prediction
...
x = np.array(x0)
y = np.array(y0)
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size=0.90, random_state=666)

model = ExtraTreesRegressor(n_estimators=60, criterion="absolute_error", max_depth=9, bootstrap=True, max_samples=0.8, random_state=777)
...
def plot_contour(ax, fixed_index, fixed_value, contour_levels_entry):
    ...
    ax.clabel(contour_lines, inline=True, fontsize=10, colors='black')  #, fmt='%.2f', inline_spacing=5
